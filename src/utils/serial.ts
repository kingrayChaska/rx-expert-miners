 ...
 *  `required` is true. Otherwise it must be 3-64 chars of [A-Z0-9-_./]. */
export const validateSerial = (
  ...
  if (value.length < 3) return { ok: false, value, error: 'Serial too short (min 3)' };
  if (value.length > SERIAL_MAX) return { ok: false, value, error: `Serial too long (max ${SERIAL_MAX})` };
